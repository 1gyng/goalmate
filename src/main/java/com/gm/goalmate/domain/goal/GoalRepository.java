...
public interface GoalRepository extends JpaRepository<Goal, Long> {
    ...
    @Query(value = "SELECT COALESCE(d.emotion, 'UNRECORDED') AS name, ROUND(SUM(CASE WHEN g.result = 'SUCCESS' THEN 1 ELSE 0 END) * 100.0 / COUNT(g.goal_id), 1) AS success_rate FROM goal g " +
